...
def datafit(reg, X, Z): 
    ...
    # using rest of the dataset as a validation set
    X1, z1 = X[l1:], Z[l1:]
    R = reg.score(X1, z1)
...
def generate_data(x):
    ...
    """ convert lines read from input file in (which contains all the input data in raw form) 
...
def output_generator(X):
    ...
    ## given an input, find the output from the blackbox function
...
for i in range(138, 150):
    ...
    data = str(ex[i][0]) + ".csv"
    numOfVar = int(ex[i][4])
    block_size = int(numOfVar/10) + 1

    start = time.time()
    z, feval = main(N, UB, LB, data, z_star)
    end = time.time()
    duration = end - start

    print("results: ", z, feval, duration)

    f.write(str(compileFile))
    f.write(",")
    f.write(str(z))
    f.write(",")
    f.write(str(feval))
